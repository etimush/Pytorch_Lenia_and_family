...
nca = Lenia(out_features,dt,k,n,kpc).cuda()
nca2 = Lenia(out_features,dt,k,n,kpc).eval().cuda().requires_grad_(False)
...
x= torch.zeros((1,out_features,full_screen_sim_y,full_screen_sim_x),dtype=torch.float16).cuda()
random_inint = torch.rand((1,out_features,52,52),dtype=torch.float16).cuda()
